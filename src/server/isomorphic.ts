...
const setDefaultState = (context: IServerContext, store: Store<IState>) => {
  ...
  state = PersistCookieStorage.getMergedStateFromServerContext<IState>(
    cookies,
    state
  );
...
const setI18nDefaultValues = (store: Store<IState>, i18n: VueI18n) => {
  ...
  try {
    defaultMessages = DEVELOPMENT
      ? JSON.parse(
          fs.readFileSync(path.resolve(`./i18n/${lang}.json`)).toString()
        )
...
export default (context: IServerContext) => {
  return new Promise((resolve: any, reject: any) => {
    ...
    router.onReady(async () => {
      ...
      const routeMatchesCatchAll = router.currentRoute.matched.some(
        (match) => match.path === "*"
      );
      ...
      const matchedComponents: Component[] = [App as Component].concat(
      ...
      try {
        ...
        const pendingPath =
          router.history.pending && router.history.pending.fullPath;

        if (
          currentPath !== context.url ||
          (pendingPath && pendingPath !== context.url)
        ) {
          reject({
            code: 302,
            cookies: PersistCookieStorage.getCookiesFromState(
              context.cookies,
              context.state
            ),
