...
const getErrorWithProbability = (probability: number) =>
  getIntInRange(0, 100) <= probability;
...
export const DemoRoutes = (app: Express.Application) => {
  ...
  app.post("/token", (req: Request, res: Response) => {
    if (getErrorWithProbability(10)) {
      ...
      res
        .status(200)
        .json({ access_token: "accessToken2", refresh_token: "refreshToken2" });
    } else if (
      req.body.grant_type === "refresh_token" &&
      req.body.refresh_token === "refreshToken2"
    ) {
      res
        .status(200)
        .json({ access_token: "accessToken", refresh_token: "refreshToken" });
