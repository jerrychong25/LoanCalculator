...
const vuesionConfig: any = JSON.parse(
  fs.readFileSync(resolve("../../.vuesion/config.json")).toString()
);
...
export const SSRRoutes = (app: Express.Application): any => {
  ...
  app.get("*", (req: Request, res: Response) => {
    ...
    if (!renderer) {
      return res
        .status(503)
        .end("waiting for compilation... refresh in a moment.");
    }
    ...
    const errorHandler = (err: any) => {
      if (err && err.code === 404) {
         ...
         */
        err.cookies.forEach((cookie: any) =>
    ...
    const render = (url: string, redirect: boolean = false): void => {
      ...
      renderer
        ...
        .on("end", () =>
          Logger.debug(`whole request: ${Date.now() - startTime}ms`)
        )
        .pipe(res);
    };
